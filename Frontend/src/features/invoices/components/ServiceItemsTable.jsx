    ...
    <div className="bg-white dark:bg-slate-900 rounded-[24px] shadow-[0_8px_30px_-12px_rgba(0,0,0,0.06)] border dark:border-slate-700 border-gray-100 dark:border-slate-800 overflow-hidden flex flex-col">
      ...
      <div className="px-6 py-5 border-b dark:border-slate-700 border-gray-100 dark:border-slate-800 flex justify-between items-center bg-gray-50/50 dark:bg-slate-800/30">
        ...
        <div className="flex items-center gap-3">
          {mode !== "credit-note" && (watch("excludedAdjustmentConnectionIds") || []).length > 0 && (
            <button
              type="button"
              title="Bring back the prorata adjustment row(s) you removed for this invoice"
              onClick={() => {
                setValue("excludedAdjustmentConnectionIds", []);
                invalidatePreview();
              }}
              className="flex items-center gap-2 px-4 py-2 rounded-full text-xs font-semibold text-gray-600 dark:text-slate-400 bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors"
            >
              Restore {(watch("excludedAdjustmentConnectionIds") || []).length} removed prorata adjustment{(watch("excludedAdjustmentConnectionIds") || []).length > 1 ? "s" : ""}
            </button>
          )}
          <button
            type="button"
                  ...
                  <tr className={`transition-colors hover:bg-gray-50/50 ${isSelected ? 'bg-white dark:bg-slate-900' : 'bg-gray-50 dark:bg-slate-800/50 opacity-40'}`}>
                    ...
                    <td className="px-2 py-4 align-middle text-center">
                      {(mode === "credit-note" || sourceType !== "CONNECTION") && (
                        <button
                          type="button"
                          title={sourceType === "PRIOR_PERIOD_ADJUSTMENT"
                            ? "Remove this prorata adjustment — it won't be regenerated for this invoice"
                            : undefined}
                          onClick={() => {
                            // A Prior Period Adjustment row is auto-regenerated by the engine on
                            // every preview/save, so just removing it from the field array isn't
                            // enough — it would reappear on the next preview. Persist the
                            // connection as excluded so the backend stops generating it here.
                            if (sourceType === "PRIOR_PERIOD_ADJUSTMENT" && mode !== "credit-note") {
                              const connectionId = item.crmConnectionSnapshot?.connectionId;
                              if (connectionId) {
                                const current = getValues("excludedAdjustmentConnectionIds") || [];
                                if (!current.includes(connectionId)) {
                                  setValue("excludedAdjustmentConnectionIds", [...current, connectionId]);
                                }
                              }
                            }
                            remove(index);
