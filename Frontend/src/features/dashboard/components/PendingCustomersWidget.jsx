...
export const PendingCustomersWidget = () => {
  ...
  if (isLoading) {
    return (
      <div className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden shadow-sm mt-8">
        {/* Skeleton Header */}
  ...
  if (isError) {
    return (
      <div className="w-full bg-red-50 dark:bg-red-500/10 border dark:border-slate-700 border-red-200 dark:border-red-500/20 rounded-xl p-6 flex items-center text-red-600 dark:text-red-400 shadow-sm mt-8">
  ...
  if (pendingCustomers.length === 0) {
    return (
      <div className="w-full bg-green-50 dark:bg-green-500/10 border dark:border-slate-700 border-green-200 dark:border-green-500/20 rounded-xl p-8 flex flex-col items-center justify-center text-center shadow-sm mt-8">
        <div className="w-12 h-12 rounded-full bg-green-100 dark:bg-green-500/10 flex items-center justify-center mb-4">
            ...
            >
              <div>
                ...
                <div className="flex items-center text-xs font-semibold text-slate-500 dark:text-slate-400 mb-6">
                  <Zap className="w-3.5 h-3.5 mr-1.5 text-amber-500 dark:text-amber-400" />
              ...
              <button
                ...
                className="w-full flex items-center justify-center px-4 py-2 bg-white dark:bg-slate-900 border dark:border-slate-700 border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 text-sm font-semibold rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-[#EA580C] dark:hover:text-[#EA580C] transition-colors group cursor-pointer"
              >
                Generate Invoice
                <ChevronRight className="w-4 h-4 ml-1 text-slate-400 dark:text-slate-500 group-hover:text-[#EA580C] transition-colors" />
              </button>
            </div>
          ))}
        </div>
      ...
      <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-3 py-2 text-sm rounded-lg border dark:border-slate-700 border-slate-300 disabled:opacity-50 disabled:cursor-not-allowed hover:border-[#EA580C] dark:border-slate-600 dark:text-slate-300"
          >
            Previous
          </button>
