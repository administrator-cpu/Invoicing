import { Mail, CheckCircle, AlertTriangle, Clock3, } from "lucide-react";
...
const CreditNoteEmailCard = ({ creditNote, onViewHistory }) => {
  const status = creditNote.email?.status || "NOT_SENT";
      ...
      <div className="p-5 sm:p-6">
        ...
        <div className="mt-6">
          ...
          <p className="text-sm font-medium text-slate-800 dark:text-slate-200">
            {creditNote.email?.lastSentAt
              ? new Date(creditNote.email.lastSentAt).toLocaleString()
              : "Never"}
          </p>
        </div>
      </div>
...
export default CreditNoteEmailCard;
